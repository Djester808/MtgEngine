...
public sealed class CommanderStatsService : ICommanderStatsService
{
    ...
    public CommanderStatsService(MtgEngineDbContext context, ICardLookup scryfall)
    {
        _context = context;
        _scryfall = scryfall;
    }
    ...
    public async Task<CommanderCardsDto> GetCommanderCardsAsync(string oracleId, int limit = 100)
    {
        ...
        foreach (var item in cardCounts)
        {
            ...
            if (cardDef == null)
                continue;
            if (cardDef.CardTypes.HasFlag(MtgEngine.Domain.Enums.CardType.Land))
                continue;
    ...
    private static int EstimateBracket(List<string> tags)
    {
        ...
        {
            if (tag is "competitive" or "optimized")
                return 4;
            if (tag is "spicy")
                return 4;
            if (tag is "tuned" or "upgraded" or "focused" or "streamlined" or "refined")
                return 3;
            if (tag is "budget" or "casual")
                return 2;
        }
        return 3;
    }
